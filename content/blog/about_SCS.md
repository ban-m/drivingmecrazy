...
\\( \S \# 1 \\)から始めて、\\( STD(1,p_2),p_2\#\bar{p_2},STD(p_2,p_3),\cdots,STD(p_{n-1},n),n\#\dollar \\)を重なりの長さ2で重ねると、
...
\begin{itemize}
    \item それぞれの\\( STD(\_,\_) \\)は重ならないので、長さは合わせて\\( \sum_{v\in 1..v-1}2OUT(v)+2 = 2m+2(n-1) \\)
	    ...
	    \item \\( \S \# 1,n\#\dollar \\)は\\( \S \#,\#\dollar \\)の分で4
	    \end{itemize}
...
また、実際には、\\( \S,\dollar \\)という2つの文字列は\\( T \\)以外には出現しないので、\\( T=\{\S 1\#,\#n\dollar\} \\)は最大でも長さ2の重複しか持たず、この時、\\( \S1\# \\)は文字列\\( P \\)の最初に、\\( \#n\dollar \\)は文字列\\( P \\)の最後になる。
